...
fn main() {
    ...
    let asset_dir = out_dir.join("moneymq-studio-ui");

    println!("cargo:warning=------------ Studio Build Script ------------");
    // Skip if already extracted
    if !asset_dir.join("_next").exists() {
        println!(
            "cargo:warning=Extracting MoneyMQ Studio UI assets to {}",
            asset_dir.display()
        );
        let url = "https://txtx-public.s3.amazonaws.com/moneymq-studio-ui/latest.zip";
